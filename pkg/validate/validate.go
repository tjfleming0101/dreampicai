...
func Password() RuleSet {
	return RuleSet{
		Name: "password",
		ValidateFunc: func(set RuleSet) bool {
			password, ok := set.FieldValue.(string)
			if !ok { 
				return false
			}
		_, ok = ValidatePassword(password)
			return ok
		},
		MessageFunc: func(set RuleSet) string {
			password, _ := set.FieldValue.(string)
			message, _ := ValidatePassword(password)
			return message
		},
	}
}
...
func Equal(s string) RuleFunc {
	return func() RuleSet {
		return RuleSet{
			...
			MessageFunc: func(set RuleSet) string {
				return fmt.Sprintf("%s and %s are not equal", set.FieldName, s)
			},
		}
	}
}
...
// IsValidPassword checks if the password is strong and meets the criteria:
